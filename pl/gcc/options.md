...
- `-Wall -Wextra -Werror`: 没有特殊理由建议编译任何程序都将这三个参数加上，提前暴露可能的诡异问题。比如C++里面定义除了main函数之外的函数如果定义了返回类型却没有`return`语句，则属于未定义行为。默认只是默默的给出一条warning，容易被忽略（问题描述参考[这里](https://stackoverflow.com/questions/1610030/why-does-flowing-off-the-end-of-a-non-void-function-without-returning-a-value-no)）。
